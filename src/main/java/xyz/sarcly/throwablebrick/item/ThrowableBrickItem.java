...
public class ThrowableBrickItem extends RangedWeaponItem{
	...
	private boolean isEnchanted(ItemStack s) { return EnchantmentHelper.getLevel(Enchantments.POWER, s) > 0 || EnchantmentHelper.getLevel(Enchantments.PUNCH, s) > 0 || EnchantmentHelper.getLevel(Enchantments.FLAME, s) > 0 || EnchantmentHelper.getLevel(Enchantments.INFINITY, s) > 0; }
    ...
    @Override public Predicate<ItemStack> getProjectiles() { return PROJECTILE; }
	@Override public int getRange() { return 15; }
    ...
    @Override public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int useTicks) {
        ...
        if (!world.isClient) {//<<Server Side>>
        	...
        	brickEnt.setRotation(90.0f, 90.0f, 0.0f);
        	//brickEnt.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), world.random.nextFloat()*7f, world.random.nextFloat()*7f, world.random.nextFloat()*7f, pullProgress*1.6f, 1.0f);
        	brickEnt.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 10f, 0f, 0f, pullProgress*1.6f, 1.0f);
